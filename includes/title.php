...
function crb_get_title() {
	...
	if ( is_home() || ( is_single() && get_post_type() == 'post' ) ) {
		...
		$title = get_the_title();
	} elseif ( is_singular( 'crb_agent' ) ) {
		$agent_obj = get_post_type_object( 'crb_agent' );

		if ( ! empty( $agent_obj ) && ! empty( $agent_obj->labels ) && ! empty( $agent_obj->labels->singular_name ) ) {
			$title = $agent_obj->labels->singular_name;
		}
	}
